...
function SignInInner() {
  ...
  const [loading, setLoading] = useState<'magic' | 'password' | 'google' | null>(null)
          ...
          <div className="bg-stone-900/60 backdrop-blur-xl border border-stone-800/80 rounded-3xl px-6 sm:px-8 py-8 shadow-[0_0_60px_-15px_rgba(245,158,11,0.15)]">
            ...
            <button
              ...
              onClick={async () => {
                setLoading('google')
                setError('')
                try { await signIn('google', { callbackUrl: '/dashboard' }) } catch { setError('Could not sign in with Google.'); setLoading(null) }
              }}
              disabled={loading !== null}
              className="w-full h-12 mb-3 flex items-center justify-center gap-2.5 bg-white hover:bg-stone-100 text-stone-900 font-semibold text-sm rounded-xl transition-colors disabled:opacity-50"
            >
              {loading === 'google' ? (
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-stone-600 text-[10px] uppercase tracking-widest">or {isSignUp ? 'sign up' : 'sign in'} with email</span>
              <div className="flex-1 h-px bg-stone-800" />
            </div>
